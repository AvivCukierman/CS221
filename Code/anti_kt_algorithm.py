...
def distIJ(ptI, etaI, phiI, mI, ptJ, etaJ, phiJ, mJ, R):
...
def antikt_algorithm(event,truth=False,inclusive=True,area=True,eta_cutoff=-1,debug=False):
...
#                               otherParticle['pt'], otherParticle['eta'], otherParticle['phi'], otherParticle['m'])
            if j < i:
                dIJ.put((distIJ(particle['pt'], particle['eta'], particle['phi'], particle['m'],
                               otherParticle['pt'], otherParticle['eta'], otherParticle['phi'], otherParticle['m'],R),
                         (i,j)))
            ...
            for iphi in range(nphi):
                ...
                for k in pseudojets:
                    ...
                    dIJ.put((distIJ(pt, eta, phi, m,
                                    pseudojet['pt'], pseudojet['eta'], pseudojet['phi'], pseudojet['m'],R),
...
def main():
  ...
  for i, event in enumerate(particle_vars):
      ...
      jets_in_event = antikt_algorithm(event,truth=truth,inclusive=inclusive,area=area,eta_cutoff=eta_cutoff,debug=debug)
